...
$status=2;
$cancelby='a';
$sql = "UPDATE reserva SET status=:status,CancelledBy=:cancelby WHERE  BookingId=:bid";
...
$status=1;
$cancelby='a';
$sql = "UPDATE reserva SET status=:status WHERE BookingId=:bcid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
<html>
<head>
<title>CHC | Admin manage Bookings</title>
				...
				<div class="agile-tables">
					<div class="w3l-table-info">
					  <h2>Manage Bookings</h2>
						...
						<tbody>
<?php $sql = "SELECT reserva.BookingId as bookid,usuario.FullName as fname,usuario.MobileNumber as mnumber,usuario.EmailId as email,paquete.PackageName as pckname,reserva.PackageId as pid,reserva.FromDate as fdate,reserva.ToDate as tdate,reserva.Comment as comment,reserva.status as status,reserva.CancelledBy as cancelby,reserva.UpdationDate as upddate from usuario join  reserva on  reserva.UserEmail=usuario.EmailId join paquete on paquete.PackageId=reserva.PackageId";
$query = $dbh -> prepare($sql);
$query->execute();
