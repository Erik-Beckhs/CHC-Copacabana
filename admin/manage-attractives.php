				...
				<div class="col-md-12">
					  ...
					  <div class="collapse navbar-collapse" id="navbarNavDropdown">
						...
						<li class="nav-item" style="width:440px;">
							<a class="nav-link text-center <?= $est==1?'active text-primary':'text-white'?>
							" href="manage-attractives.php?estado=1">Activos</a>
						</li>
						<li class="nav-item mx-1" style="width:440px;">
							<a class="nav-link text-center <?= $est==0?'active text-primary':'text-white'?>" href="manage-attractives.php?estado=0">Inactivos</a>
						</li>
						</ul>
					</div>
</nav>
...
<?php
$regpp=8;
$sq="select * from atractivo where activo=$est";
$quer=$dbh->prepare($sq);
$quer->execute();
$total=$quer->rowCount();
...
<div class="justify-content-right">
					  ...
					  <input type="hidden" name="pagina" value="1">
						<input class="form-control" type="text" name="busqueda" placeholder="Buscar" id="busqueda">
...
{				?>		
						  <tr>
							...
							<form action="includes/controlador_atr.php" method="POST">
								...
								<input type="hidden" value=<?= $est; ?> name="estado" id="estado">
								<input type="submit" class="button rounded h5 text-white <?= $est==1? 'bg-danger' : 'bg-success' ?>" name="cambiar_estado" value=<?=
								$est==1? Inactivar : Activar?> id="estadoA">
							</td>
							<td>
							<div class="h4 text-center">	
								<a href="update.php?uid=<?= htmlentities($result->ID);?>" title="Modificar"><i class="fa fa-edit text-info"></i></a>
